...
To examine the depth and breadth of laterality in this dataset we compared LIs at the participant level using an independent samples *t*-test and then examined the proportion using a *z*-test. We note that there were few left handers in this data set so the analysis should be appraoched with caution. 
...
As with the jabberwocky data, we compare LIs at the population level between right and left handers in both data sets (Session 1 data and Session 2 data) using a series of independent samples t-tests and apply a bonferroni adjustment to these NHST to correct for mulitple comparions.
